...
app.post('/new', upload.fields([{ name: 'image' }, { name: 'imgOne' }]), async (req, res) => {
  ...
  const image = req.files['image'] ? `http://13.60.250.221:3000/${req.files['image'][0].path.replace(/\\/g, '/')}` : ''
  const imgOne = req.files['imgOne'] ? `http://13.60.250.221:3000/${req.files['imgOne'][0].path.replace(/\\/g, '/')}` : ''

  // Log the specific values for debugging
  console.log(`Heading: ${heading}, About: ${about}, Location: ${loction}, Dates: ${dates}`);

  try {
    // Create a new blog instance
    const blog = new Blog({ heading, headingOne, aboutOne, about, image, imgOne, loction, dates });
...
app.post('/admin/edit/:id', upload.fields([{ name: 'image' }, { name: 'imgOne' }]), async (req, res) => {
  ...
  if (req.files) {
    if (req.files.image && req.files.image.length > 0) {
      updatedData.image = req.files['image'] ? `http://13.60.250.221:3000/${req.files['image'][0].path.replace(/\\/g, '/')}` : '' // Ensure proper path format
    }
    if (req.files.imgOne && req.files.imgOne.length > 0) {
      updatedData.imgOne = req.files['imgOne'] ? `http://13.60.250.221:3000/${req.files['imgOne'][0].path.replace(/\\/g, '/')}` : '' // Ensure proper path format
    }
  }
